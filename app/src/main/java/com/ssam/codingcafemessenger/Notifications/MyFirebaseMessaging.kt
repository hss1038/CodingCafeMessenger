...
class MyFirebaseMessaging : FirebaseMessagingService() {
    override fun onMessageReceived(mRemoteMessage: RemoteMessage) {
        ...
        val currentOnlinUser = sharedPref.getString("currentUser", "none")
        val firebaseUser = FirebaseAuth.getInstance().currentUser
        if(firebaseUser != null && sented == firebaseUser.uid){
            if(currentOnlinUser != user){
                if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.O){
                    sendOreoNotification(mRemoteMessage)
                } else {
                    sendNofitication(mRemoteMessage)
                }
            }
        }
    }

    private fun sendNofitication(mRemoteMessage: RemoteMessage) {
        val user = mRemoteMessage.data["user"]
